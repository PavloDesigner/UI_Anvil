...
export function initPreviews() {
  ...
  subscribeTypography(() => { if (currentTab === 'typography') render(); });
  render();
}
...
function buildMktDonut(hasS, hasT) {
  ...
  const svg = `<svg class="mkt-ring-svg" viewBox="0 0 90 90" fill="none" aria-hidden="true">
    ...
    ${circles.join('')}
    <text x="${cx}" y="${cy - 3}" text-anchor="middle" font-size="14" font-weight="800" style="fill:var(--mkt-dark-text)" font-family="sans-serif">${segs.length}</text>
    <text x="${cx}" y="${cy + 11}" text-anchor="middle" font-size="7.5" style="fill:var(--mkt-dark-muted)" font-family="sans-serif">palette${segs.length !== 1 ? 's' : ''}</text>
  </svg>`;
      ...
      <div class="mkt-profile-inner">
        <div class="mkt-profile-avatar-wrap">
          <svg class="mkt-profile-avatar" viewBox="0 0 52 52" fill="none" aria-hidden="true">
            ...
            <text x="26" y="31" text-anchor="middle" font-size="16" font-weight="700" fill="rgba(255,255,255,0.92)" font-family="sans-serif">LP</text>
          </svg>
